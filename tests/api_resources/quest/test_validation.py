# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from farquest import Farquest, AsyncFarquest

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestValidation:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_retrieve(self, client: Farquest) -> None:
        validation = client.quest.validation.retrieve(
            "string",
        )
        assert validation is None

    @parametrize
    def test_raw_response_retrieve(self, client: Farquest) -> None:
        response = client.quest.validation.with_raw_response.retrieve(
            "string",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        validation = response.parse()
        assert validation is None

    @parametrize
    def test_streaming_response_retrieve(self, client: Farquest) -> None:
        with client.quest.validation.with_streaming_response.retrieve(
            "string",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            validation = response.parse()
            assert validation is None

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_retrieve(self, client: Farquest) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            client.quest.validation.with_raw_response.retrieve(
                "",
            )


class TestAsyncValidation:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_retrieve(self, async_client: AsyncFarquest) -> None:
        validation = await async_client.quest.validation.retrieve(
            "string",
        )
        assert validation is None

    @parametrize
    async def test_raw_response_retrieve(self, async_client: AsyncFarquest) -> None:
        response = await async_client.quest.validation.with_raw_response.retrieve(
            "string",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        validation = await response.parse()
        assert validation is None

    @parametrize
    async def test_streaming_response_retrieve(self, async_client: AsyncFarquest) -> None:
        async with async_client.quest.validation.with_streaming_response.retrieve(
            "string",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            validation = await response.parse()
            assert validation is None

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_retrieve(self, async_client: AsyncFarquest) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `id` but received ''"):
            await async_client.quest.validation.with_raw_response.retrieve(
                "",
            )
